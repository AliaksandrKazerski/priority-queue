...
class MaxHeap {
	...
	pop() {
		...
		if (this.root !== null) {
			...
			this.parentNodes = correctState([this.root]);
		}
	...
	detachRoot() {
		for (let i = 0; i < this.parentNodes.length; i++) {
			if (this.parentNodes[i].priority === this.root.priority && this.parentNodes[i].data === this.root.data) {
	...
	restoreRootFromLastInsertedNode(detached) {
		...
		if (detached.left !== null) {
			...
			this.root = temp;
			if (detached.parent !== null && detached.parent.left !== null && detached.parent.left === detached) {
				detached.parent.left = null;
			}
			if (detached.parent !== null && detached.parent.right !== null && detached.parent.right === detached) {
